...
- Playwright を用いた簡易スモークテスト `tests/feature_smoke_playwright.py` を追加しました。`python -m http.server 8000` などでローカルサーバーを起動した状態で `python tests/feature_smoke_playwright.py` を実行すると、ヒーローセクション表示、ステージヘッダー、ヒントボタン、矢印キー移動、手動採点、プレイヤー入力の色分けといった主要要件を一括で検証できます。
